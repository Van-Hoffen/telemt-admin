...
Используйте команду `/token` для генерации приглашений:

- `/token create [days]` — создать токен с ручным подтверждением (по умолчанию 14 дней).
...
- `⚙️ Статус сервиса` — управление `telemt.service` (Start/Stop/Restart).
